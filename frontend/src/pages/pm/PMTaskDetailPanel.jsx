...
export default function PMTaskDetailPanel({
  ...
  const isBlocked =
    Boolean(dependencies?.is_blocked ?? dependencies?.blocked ?? pendingBlockers.length > 0) ||
          ...
          {scheduleSuggestion?.fuera_de_secuencia ? (
            <div className="pm-task-schedule-panel">
              ...
              <div className="pm-task-impact-note">
                ...
                <span>{safeDisplayText(scheduleSuggestion.razon, "La tarea inicia antes de que termine su prerrequisito.")}</span>
              </div>
            </div>
          ) : null}
                    ...
                    <div className="pm-detail-list-item" key={dependency.id}>
                      <div>
                        ...
                        <span>
                          ...
                          {safeDisplayText(
                            getTaskStatusLabel(dependency.resolved_status ?? dependency.depende_de_tarea_estatus),
                            dependency.resolved_status ?? dependency.depende_de_tarea_estatus,
                          )}
                        </span>
                      </div>
  ...
  const detailContent = embedded ? (
    <>
      <div className="pm-task-detail-inline-actions">{detailActions}</div>
      {detailBody}
    </>
  ) : (
    <DataCard actions={detailActions} subtitle="Información completa de la tarea seleccionada." title="Detalle de tarea">
      {detailBody}
    </DataCard>
  );
