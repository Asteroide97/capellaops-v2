...
| POS | Ticket por email | Implementado MVP | Media | POS F5 | El ticket puede enviarse por email en HTML y registra el intento en `pos_ticket_deliveries`; requiere `SENDGRID_API_KEY` y `EMAIL_FROM`. | El cliente no recibe comprobante digital fuera de impresión local. |
| POS | Ticket por SMS | Implementado MVP | Media | POS F5 | El ticket puede enviarse por SMS resumido y registra trazabilidad; requiere `TWILIO_ACCOUNT_SID`, `TWILIO_AUTH_TOKEN` y `TWILIO_SMS_FROM`. | Seguimiento comercial débil cuando el cliente solo pide confirmación móvil. |
| POS | Link publico de ticket | Pendiente | Media | POS futura | No implementado en esta fase; el SMS usa resumen corto sin URL pública. | No hay enlace reutilizable del comprobante fuera de la app. |
