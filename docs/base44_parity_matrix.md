...
| Inventario | Resumen / Dashboard de inventario | Implementado | Alta | Inventario Fase 2 | Dashboard operativo con KPIs, indicadores, alertas accionables, bajo stock, materiales sin precio, materiales sin costo, rotacion y movimientos recientes. | Operacion sin visibilidad ejecutiva. |
...
| Inventario | Alertas de inventario | Implementado MVP | Alta | Inventario Fase 2 | `GET /inventory/summary` ya calcula alertas operativas de bajo stock, sin stock, sin precio de venta, sin costo, requisiciones, compras pendientes y ventas sin margen positivo. | Riesgos operativos invisibles si el dashboard no concentra incidencias. |
| Inventario | Bajo stock operativo | Implementado MVP | Alta | Inventario Fase 2 | El resumen operativo lista materiales bajo minimo, cantidad sugerida, requisicion pendiente y accion directa para crear requisicion sin duplicados. | Reposicion lenta y quiebres de stock evitables. |
| Inventario | Materiales sin precio de venta | Implementado MVP | Alta | Inventario Fase 2 | El resumen y Materiales ya detectan `precio_venta` nulo o cero y permiten navegar para corregir el catalogo usado por POS. | Ventas bloqueadas o con precio capturado manualmente. |
| Inventario | Materiales sin costo | Implementado MVP | Alta | Inventario Fase 2 | El resumen y Materiales ya detectan ausencia simultanea de `costo_unitario` y `costo_promedio_actual`, para proteger margen y utilidad estimada. | Margen operativo y reportes de utilidad distorsionados. |
...
| Inventario | Rotacion basica | Implementado MVP | Media | Inventario Fase 2 | El resumen operativo ya expone productos mas movidos, baja rotacion por ventana configurable y ultimos movimientos confirmados. | Inventario inmovil o comportamiento atipico sin visibilidad rapida. |
| Inventario | Reportes avanzados | Pendiente | Media | Inventario F3 | Ya existe resumen operativo accionable, pero siguen pendientes graficas, exportaciones y analitica avanzada dedicada. | Operacion sin analitica profunda. |
...
| POS | Corte de caja | Implementado MVP | Alta | POS F5B | El turno ya se puede abrir, operar y cerrar con efectivo contado, diferencia, notas y corte consultable. | Riesgo operativo si no se endurecen arqueos y auditoría avanzada. |
...
| POS | Historial de turnos | Implementado MVP | Media | POS F5B | `Caja / Turnos` ya consulta turnos abiertos y cerrados con filtros, diferencias y acceso al reporte. | Sin historial operativo la supervisión diaria queda fragmentada. |
| POS | Corte imprimible | Implementado MVP | Media | POS F5B | El corte de caja ya tiene modal formal e impresión dedicada separada del ticket. | La salida documental del turno sigue débil si no existe un corte claro. |
| POS | Corte de caja avanzado | Parcial | Media | POS F5B | Ya existe corte formal, historial y diferencia de efectivo, pero faltan auditoría avanzada y exportaciones estructuradas. | Cierres operativos todavía limitados en escenarios complejos. |
| POS | Reportes POS | Implementado MVP | Alta | POS F6 | `POS > Reportes` ya consolida KPIs, ventas por periodo, métodos de pago, productos más vendidos, cancelaciones y utilidad estimada con filtros operativos. | La dirección pierde visibilidad comercial diaria si no existe un resumen confiable. |
| POS | Productos más vendidos | Implementado MVP | Media | POS F6 | El reporte ya agrupa cantidades y total vendido por material dentro del rango filtrado. | Surtido y exhibición operan sin claridad de rotación. |
| POS | Ventas por cajero | Implementado MVP | Media | POS F6 | El reporte ya resume ventas y total neto por usuario de POS. | Supervisión de desempeño débil por turno o cajero. |
| POS | Ventas por almacén | Implementado MVP | Media | POS F6 | El reporte ya compara ventas netas por almacén operativo. | Comparativos entre sucursales o almacenes quedan fuera del sistema. |
| POS | Utilidad estimada | Implementado MVP | Media | POS F6 | Se calcula con ingreso neto menos costo estimado por línea usando snapshot de inventario o fallback al costo del material. | La operación vende sin referencia rápida de margen estimado. |
| POS | Solicitud de factura desde POS | Implementado MVP | Alta | POS F7 | Una venta pagada ya puede capturar solicitud de factura y guardar datos fiscales basicos sin timbrar CFDI. | La venta sigue desconectada del flujo fiscal si no existe una preparación previa. |
| POS | Captura de datos fiscales POS | Implementado MVP | Alta | POS F7 | POS ya guarda RFC, razón social, email, uso CFDI, régimen fiscal, código postal y notas dentro de la venta para preparación fiscal futura. | La captura fiscal queda fuera del sistema y se vuelve a pedir después. |
| POS | Facturación POS pendiente de timbrado | Implementado MVP | Media | POS F7 | `POS > Facturación` ya lista solicitudes en `pendiente_datos`, `lista_para_facturar` y puede enlazar la solicitud a la bandeja fiscal. | Sin enlace entre POS y revisión fiscal, la preparación se fragmenta. |
| Facturacion | Bandeja fiscal POS | Implementado MVP | Alta | Facturacion F1 | La vista `Facturacion > Solicitudes POS` ya centraliza solicitudes de POS para revisión y preparación operativa previa a CFDI. | La revisión fiscal seguiría dispersa entre ticket, historial y mostrador. |
| Facturacion | Validación fiscal básica | Implementado MVP | Alta | Facturacion F1 | La bandeja ya valida RFC, razón social, email, uso CFDI, régimen fiscal, código postal y estado pagado de la venta sin conectarse al SAT. | Se intentaría preparar información incompleta para timbrado futuro. |
| Facturacion | Preparación CFDI sin timbrado | Implementado MVP | Alta | Facturacion F1 | La acción `Preparar para timbrado futuro` deja la solicitud en `preparada` sin XML, sin PDF fiscal y sin integración con Factura.com. | Riesgo de asumir erróneamente que ya existe CFDI emitido. |
| POS | Timbrado CFDI desde POS | Pendiente | Alta | Facturacion POS futura | Esta fase no timbra CFDI ni marca ventas como `facturada`. | La venta sigue sin cierre fiscal real. |
| POS | Factura global POS | Pendiente | Media | Facturacion POS futura | `factura_requiere_factura_global` queda preparado pero el flujo global no se implementa en esta fase. | Operación mostrador sigue sin resolver ventas de público general a corte fiscal. |
| POS | Factura.com integración | Pendiente | Alta | Facturacion POS futura | No existe conexión con Factura.com ni proveedores fiscales externos desde POS. | La preparación fiscal no llega todavía al timbrado operativo. |
| POS | Exportación avanzada PDF/Excel | Pendiente | Baja | POS futura | Solo existe exportación CSV simple desde la vista de reportes. | La salida analítica formal sigue limitada. |
...
| POS | Ticket imprimible formal | Implementado MVP | Media | POS F5A | El ticket ya incluye productos, descuentos, pagos, cambio y marca de cancelación con CSS de impresión dedicada. | Entrega de comprobantes débil ante clientes. |
| POS | Ticket por email | Pendiente | Media | POS futura | No implementado en esta fase. | El cliente no recibe comprobante digital fuera de impresión local. |
| POS | Ticket por SMS | Pendiente | Media | POS futura | No implementado en esta fase. | Seguimiento comercial débil cuando el cliente solo pide confirmación móvil. |
| POS | Link publico de ticket | Pendiente | Media | POS futura | No implementado en esta fase. | No hay enlace reutilizable del comprobante fuera de la app. |
...
| Facturacion | Facturacion pendiente / bloqueada en V2 | Parcial | Alta | Facturacion F1 | La facturacion real sigue bloqueada, pero ya existe bandeja de revisión previa para solicitudes POS. | Puede confundirse preparación operativa con facturación real si no se comunica bien. |
...
| PM / Inventario | Materiales vinculados PM-Inventario | Implementado MVP | Alta | Inventario-PM F1 | PM ya selecciona materiales reales y almacenes del módulo Inventario como fuente de verdad. | Materiales duplicados o inconsistentes entre PM e Inventario. |
| PM / Inventario | Consumos por proyecto | Implementado MVP | Alta | Inventario-PM F1 | Los consumos y devoluciones desde PM ya crean movimientos de inventario, descuentan o reponen stock y quedan ligados al proyecto. | Salidas de material sin trazabilidad por proyecto. |
| PM / Inventario | Kardex por proyecto | Implementado MVP | Alta | Inventario-PM F1 | El Kardex y la vista Inventario por proyecto ya muestran movimientos ligados a proyecto, tarea y partida. | Difícil auditar qué material salió para cada proyecto. |
...
| PM / Inventario | Requisiciones desde PM | Implementado MVP | Alta | Inventario-PM F2 | PM ya crea requisiciones ligadas a proyecto, tarea y partida, y las consulta desde la tab de materiales del proyecto. | La necesidad de materiales vuelve a salir del sistema si no existe solicitud formal. |
| PM / Inventario | Aprobación de requisiciones | Implementado MVP | Alta | Inventario-PM F2 | Inventario ya puede aprobar o rechazar requisiciones de proyecto con cantidades aprobadas por línea y motivo de rechazo. | Solicitudes sin control operativo o sin trazabilidad de aprobación. |
| PM / Inventario | Surtido parcial de requisiciones | Implementado MVP | Alta | Inventario-PM F2 | El surtido ya permite cantidades parciales por línea, deja saldo pendiente y mantiene la requisición en estado parcial cuando falta stock. | El abastecimiento se frena por completo aunque exista stock parcial utilizable. |
| PM / Inventario | Órdenes de compra desde requisición | Pendiente | Media | Inventario-PM F3 | No existe todavía continuidad automática desde requisición de proyecto hasta orden de compra. | Flujo operativo fragmentado entre PM e inventario. |
| PM / Inventario | Compras avanzadas desde requisición PM | Pendiente | Media | Inventario-PM F3 | La requisición de proyecto aún no baja a abastecimiento avanzado con proveedor, recepción y seguimiento comercial completos. | El faltante pendiente sigue resolviéndose fuera del flujo integrado. |
...
| PM / Inventario | Lotes / series en consumos PM | Pendiente | Media | Inventario-PM futura | Los consumos por proyecto todavía no manejan lotes, series ni caducidades. | Trazabilidad física limitada para materiales controlados. |
...
| PM | Reporte ejecutivo PM | Implementado MVP | Media | PM F11 | Ya existe vista ejecutiva consolidada con KPIs globales, tabla por proyecto, riesgos, salud y cobranza PM. | La direccion pierde visibilidad rápida del portafolio si no existe esta lectura resumida. |
| PM | Riesgos PM | Implementado MVP | Media | PM F11 | El reporte ejecutivo ya lista atraso, desviación de fecha, sobrecosto, cambios pendientes, estimaciones pendientes y riesgos de ruta crítica. | Riesgos operativos dispersos o detectados tarde. |
| PM | Resumen financiero PM | Implementado MVP | Media | PM F11 | El reporte ya consolida presupuesto, costo real, estimado, aprobado, cobrado y pendiente por cobrar. | Seguimiento financiero PM sin vista concentrada. |
| PM | PM MVP Hardening | Implementado | Alta | PM F14 | El módulo PM ya endurece permisos básicos, visibilidad de acciones por estado, mensajes legibles, modales consistentes y limpieza visual para demo. | Un MVP sin hardening pierde credibilidad en demo y genera errores evitables en operación. |
| PM | Permisos finos PM | Parcial | Media | PM F14 | Ya existe gating básico por rol y estado en backend y frontend, pero no hay RBAC granular completo por entidad y política. | Acciones visibles o bloqueos pueden quedarse demasiado amplios para clientes más exigentes. |
| PM | QA checklist PM | Implementado | Media | PM F14 | Ya existe checklist manual repetible en `docs/pm_mvp_smoke_checklist.md` para dashboard, proyectos, plan de trabajo, presupuesto, estimaciones, línea base, aprobaciones, documentos y portal. | Sin checklist repetible el demo puede degradarse entre cambios menores. |
| PM | Export PDF / Excel de reportes PM | Pendiente | Media | PM futura | El reporte ejecutivo no exporta todavía a PDF o Excel. | Difícil compartir cortes formales fuera del sistema. |
| PM | Gráficas avanzadas PM | Pendiente | Baja | PM futura | El MVP prioriza cards, tabla y riesgos; la capa analítica avanzada sigue pendiente. | Menor lectura visual para comités ejecutivos. |
| PM | BI avanzado PM | Pendiente | Baja | PM futura | No existe todavía un motor BI con tendencias históricas, cubos o slicing profundo. | Analítica limitada para portafolios grandes. |
...
| Empresa | Logo empresarial | Implementado MVP | Media | Branding F1 | La empresa ya puede subir y persistir `logo_url` y `logo_blob_path` para documentos operativos. | La eliminacion fisica del blob sigue pendiente. |
| Empresa | Perfil de empresa para documentos | Implementado MVP | Media | Branding F1 | Ya existe perfil editable con nombre comercial, razon social, RFC, contacto y direccion para reutilizar en documentos y vistas. | No valida SAT ni reglas fiscales avanzadas. |
| Empresa / PM | Uso en PDF de estimaciones | Implementado MVP | Media | Branding F1 | El PDF de estimaciones ya consume `logo_url` y datos comerciales de empresa cuando existen. | Si el logo falla, usa fallback textual. |
| Empresa / Compras | Uso en PDF de ordenes de compra | Implementado MVP | Media | Branding F1 | La orden de compra PDF ya consume `logo_url` y datos comerciales de empresa cuando existen. | Si el logo falla, usa fallback textual. |
| Inventario / Compras | Recepción avanzada OC | Implementado MVP | Alta | Compras F2 | La recepción de OC ya valida pendientes, registra documento o remisión, crea entrada a inventario y conserva un evento formal por recepción. | No maneja lotes, series ni caducidades todavía. |
| Inventario / Compras | Historial de recepciones | Implementado MVP | Alta | Compras F2 | Cada orden de compra ya muestra historial formal de recepciones con usuario, almacén, documento y renglones recibidos. | Órdenes antiguas previas a la migración pueden seguir mostrando solo movimientos históricos. |
| Inventario / Compras | Reporte compras pendientes | Implementado MVP | Media | Compras F2 | Ya existe reporte operativo de órdenes abiertas, materiales pendientes y proveedores con compras por recibir. | No incluye cuentas por pagar ni conciliación financiera. |
| Inventario / Compras | PDF OC con branding | Implementado MVP | Media | Documentos PDF F1 | La exportación PDF de orden de compra mantiene branding, proveedor, renglones, totales y leyenda no fiscal. | Sigue siendo documento operativo, no fiscal. |
| Inventario | Facturación proveedor | Pendiente | Alta | Compras F3 | No implementado. | Recepción y abastecimiento sin conciliación fiscal del proveedor. |
| Empresa / POS | Uso en tickets POS | Implementado MVP | Baja | Branding QA | El ticket POS imprimible ya usa branding de empresa desde la sesion activa con logo, nombre, RFC, direccion y contacto cuando existen. | Si el logo falla, el ticket conserva encabezado textual. |
| Empresa / POS | Uso en corte de caja | Implementado MVP | Baja | Branding QA | El corte de caja imprimible ya muestra el mismo encabezado empresarial y mantiene impresion aislada del resto de la app. | Sigue dependiendo de la sesion activa para mostrar branding en frontend. |
...
| PM | Facturación real | Pendiente | Alta | PM comercial futura | PM sigue separado de facturación fiscal y cobranza real; el MVP solo registra estados internos. | Operación comercial incompleta si se espera cierre financiero dentro del mismo módulo. |
