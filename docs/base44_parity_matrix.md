...
| PM | Gantt mejorado | Implementado | Media | PM F6 | El plan de trabajo ya muestra Gantt mejorado con bloqueo, criticidad, fuera de secuencia, dependencia textual y fechas sugeridas. | Sin edicion directa ni reprogramacion visual automatica. |
...
| PM | Recalculo fechas sugeridas | Implementado | Media | PM F6 | La planeacion ya sugiere fecha de inicio y fin cuando una tarea esta programada antes de su prerrequisito. | Aun no aplica cambios automaticamente. |
...
| PM | Calendarios laborales | Pendiente | Media | PM futura | No existen calendarios laborales, feriados ni dias no laborables aplicados a la planeacion. | La ruta critica y las fechas sugeridas usan dias corridos. |
